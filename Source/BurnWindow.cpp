...
BView* BurnWindow::_CreateToolBar()
{
	...
	BMenuItem* deviceItem = new BMenuItem("No Recording Devices Found",
		new BMessage());
	...
	// TODO These values should be obtained from the capabilities of the drive
	...
	BLayoutBuilder::Group<>(groupView)
		...
		.AddGrid(kControlPadding, kControlPadding)
			...
			.Add(new BButton("BurnDiscButton", "Burn Disc",
				new BMessage(kBurnDiscMessage)), 1, 0, 1, 1)
			.Add(new BButton("BuildISOButton", "Build ISO",
				new BMessage(kBuildImageMessage)), 1, 1, 1, 1)
		.End();
...
BView* BurnWindow::_CreateDiskUsageView()
{
	...
	// TODO This StatusBar will probably be moved to a private class variable
	// to allow for easy adjustments from MessageReceived()
	...
	// TODO Send actual BMessages and readjust the max position of the
	// StatusBar

	// Create our media pop up menu items and use the 650MiB item as the
	// default
...
//	mediaMenu->AddItem(new BMenuItem("DVD9 7.95GiB", new BMessage()));
	...
	BMenuField* mediaMenuField = new BMenuField("MediaTypeMenuField",
		"Media:", mediaMenu);
	mediaMenuField->SetExplicitMaxSize(BSize(200, B_SIZE_UNLIMITED));

	BLayoutBuilder::Group<>(diskUsageView)
		.SetInsets(kControlPadding, kControlPadding, kControlPadding,
			kControlPadding)
		.Add(statusBar)
		.AddStrut(10)
		.Add(mediaMenuField);
